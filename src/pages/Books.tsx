    ...
    render = (): React.ReactNode => <AppLayout>
        <section className="flex justify-center font-bold text-stone-700">
            <Formik
                ...
                {({ setFieldValue, isSubmitting }) => <Form className="flex rounded gap-4 m-4 bg-white shadow-sm shadow-slate-900 p-4 items-center flex-col">
                    ...
                    <label htmlFor="imageBook" className="w-full">Image</label>
                    <input type="file" id="imageBook" name="image" onChange={(e: any) => setFieldValue('image', e.target.files[0])} className="w-full" />
                    <button type="submit" disabled={isSubmitting} className="btn-primary w-full h-10">{isSubmitting?<i className="fas fa-circle-notch fa-spin"></i>:'Submit'}</button>
                </Form>}
            </Formik>
        </section>
    </AppLayout>
